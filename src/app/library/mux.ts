...
export default class Mux extends Template {
    ...
    __muxTreeWidth(pitch: number, leafs: number, cw: number, valvewidth: number): number {
        // Envelope tracks the leaf fan, plus one horizontal valve past the outer channel.
        return (leafs - 1) * pitch + valvewidth + 2 * cw;
    }

    __muxValveAlong(stagelength: number, cw: number, valvelength: number): number {
        // Honor valveWidthY; only keep the pad inside the vertical stage.
    ...
    __muxValveAcross(valvewidth: number, _twigSpacing: number, cw: number): number {
        // Honor valveWidthX so the pad can grow left/right without a pitch cap.
    ...
    __muxValveCenterOffsets(stagelength: number, along: number): { left: number; right: number } {
        // Spread left/right control buses across the stage without growing
        // stageSpace. Same-stage center gap is 0.4*slack (0.3 / 0.7 of slack).
