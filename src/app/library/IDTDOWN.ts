...
export default class IDTDOWN extends Template {
    constructor() {
        super();
    }
    ...
    __renderFlow(params: { [k: string]: any }) {
        ...
        for (let i = 0; i < numArray-2; i++) {
            rec = new paper.Path.Rectangle({
                point: new paper.Point(px + i  * 2*spacing + i * w, py - 1),
                size: [w, l*.75],
                radius: 0
            });
        ...
        const topchannel = new paper.Path.Rectangle({
            ...
            size: [numArray * w + (numArray - 1.4) * spacing, w]
        });
    ...
    render2D(params: { [k: string]: any }, key = "FLOW") {
        ...
        throw new Error("Unknown render key found in IDT: " + key);
    }
