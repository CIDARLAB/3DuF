...
export default class Sorter extends Template {
  ...
  getPorts(params: { [k: string]: any }) {
    ...
    const outletLen =
      ((numberofDistributors + 0.5) * (pressureSpacing + pressureWidth)) /
  ...
  __renderIntegrate(params: { [k: string]: any }) {
    ...
    let bottomRight = new paper.Point(
      x + electrodeWidth / 2,
      y + electrodeDistance + electrodeLength
    );
    ...
    topLeft = new paper.Point(
      ...
      y +
        electrodeDistance +
        2 * electrodeWidth * Math.tan(((angle / 2) * Math.PI) / 180)
    );
    bottomRight = new paper.Point(
      x + electrodeWidth / 2 + 2 * electrodeWidth,
      y +
