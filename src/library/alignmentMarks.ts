...
export default class AlignmentMarks extends Template {
  ...
  render2DTarget(key: string, params: ParamsIF): paper.CompoundPath {
    ...
    const bottomrightpoint = new paper.Point(
      position[0] + width,
      position[1] + length
    );
    ...
    const bottomrightrect = new paper.Path.Rectangle(
      position as unknown as paper.Rectangle,
      bottomrightpoint as unknown as paper.Size
    );
    ...
    const bottommiddlepoint = new paper.Point(
      position[0],
      position[1] + length
    );
