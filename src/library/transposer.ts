...
export default class Transposer extends Template {
  ...
  getPorts(params: { [k: string]: any }) {
    ...
    ports.push(
      new ComponentPort(
        6 * valvespacing + 4 * radius + 3 * channelWidth,
        0,
        "2",
    ...
    ports.push(
      new ComponentPort(
        0,
        2 * channelWidth + 4 * valvespacing + 2 * 2 * radius,
        "3",
        LogicalLayerType.FLOW
      )
    );
    ...
    ports.push(
      new ComponentPort(
        -2 * radius - channelWidth / 2,
        channelWidth + 2 * valvespacing + 2 * radius,
        "5",
    ...
    ports.push(
      new ComponentPort(
        5 * valvespacing + 6 * radius + 3 * channelWidth,
        channelWidth + 2 * valvespacing + 2 * radius,
        "6",
        LogicalLayerType.CONTROL
      )
  ...
  __drawFlow(params: { [k: string]: any }) {
    ...
    let bottomrightpoint = new paper.Point(
      px + 4 * valvespacing + 2 * radius + 2 * channelWidth,
      py + channelWidth / 2
    );
    ...
    // Draw Valve
    this.__createTransposerValve(
      transposer_flow,
      bottomrightpoint.x + radius,
      topleftpoint.y + channelWidth / 2,
      valveGap,
      radius,
      "H",
      channelWidth
    ...
    topleftpoint = new paper.Point(
      px + 4 * valvespacing + 4 * radius + 2 * channelWidth,
      py - channelWidth / 2
    );
    bottomrightpoint = new paper.Point(
      px + 6 * valvespacing + 4 * radius + 3 * channelWidth,
      py + channelWidth / 2
    );
    ...
    topleftpoint = new paper.Point(
      px + 3 * valvespacing + channelWidth + 2 * radius,
      py + channelWidth / 2
    );
    bottomrightpoint = new paper.Point(
      topleftpoint.x + channelWidth,
      topleftpoint.y + valvespacing
    );
    channel = new paper.Path.Rectangle(topleftpoint, bottomrightpoint);
    ...
    this.__createTransposerValve(
      transposer_flow,
      topleftpoint.x + channelWidth / 2,
      bottomrightpoint.y + radius,
      valveGap,
      radius,
      "V",
      channelWidth
    ...
    topleftpoint = new paper.Point(
      topleftpoint.x,
      bottomrightpoint.y + 2 * radius
    );
    bottomrightpoint = new paper.Point(
      topleftpoint.x + channelWidth,
      topleftpoint.y + 2 * valvespacing + channelWidth
    );
    channel = new paper.Path.Rectangle(topleftpoint, bottomrightpoint);
    ...
    topleftpoint = new paper.Point(
      px,
      py + 1.5 * channelWidth + 4 * valvespacing + 2 * 2 * radius
    );
    bottomrightpoint = new paper.Point(
      topleftpoint.x + 2 * valvespacing + channelWidth,
      topleftpoint.y + channelWidth
    );
    channel = new paper.Path.Rectangle(topleftpoint, bottomrightpoint);
    ...
    // 2
    topleftpoint = new paper.Point(
      bottomrightpoint.x + 2 * radius,
      topleftpoint.y
    );
    bottomrightpoint = new paper.Point(
      topleftpoint.x + 4 * valvespacing + 2 * channelWidth + 2 * radius,
      topleftpoint.y + channelWidth
    );
    channel = new paper.Path.Rectangle(topleftpoint, bottomrightpoint);
    ...
    topleftpoint = new paper.Point(
      px + 5 * valvespacing + 2 * channelWidth + 4 * radius,
      py + channelWidth / 2
    );
    bottomrightpoint = new paper.Point(
      topleftpoint.x + channelWidth,
    ...
    bottomrightpoint = new paper.Point(
      topleftpoint.x + channelWidth,
      topleftpoint.y + valvespacing + channelWidth
    );
    channel = new paper.Path.Rectangle(topleftpoint, bottomrightpoint);
    ...
    topleftpoint = new paper.Point(
      px + valvespacing,
      py + 1.5 * channelWidth + 2 * valvespacing + 2 * radius
    );
    bottomrightpoint = new paper.Point(
      topleftpoint.x + channelWidth,
      topleftpoint.y + valvespacing
  ...
  __createTransposerValve(
    compound_path: paper.CompoundPath,
    xpos: number,
    ypos: number,
    valveGap: number,
    radius: number,
    ...
    let circ: paper.Path.Circle | paper.PathItem = new paper.Path.Circle(
      center,
      radius
    );
  ...
  __drawControl(params: { [k: string]: any }) {
    ...
    let center = new paper.Point(
      px + 4 * valvespacing + 2 * channelWidth + 2 * radius + radius,
      py
    );
    ...
    center = new paper.Point(
      px + 1.5 * channelWidth + 3 * valvespacing + 2 * radius,
    ...
    center = new paper.Point(
      center.x + 2 * valvespacing + 2 * radius + channelWidth,
      py + channelWidth / 2 + valvespacing + radius
    );
    circle = new paper.Path.Circle(center, radius);
    transposer_control.addChild(circle);
    ...
    let rect = new paper.Path.Rectangle({
      from: new paper.Point(
        crosschannelstart.x,
        crosschannelstart.y - channelWidth / 2
      ),
      to: new paper.Point(
        crosschannelend.x,
        crosschannelstart.y + channelWidth / 2
    ...
    bottomRight = new paper.Point(
      crosschannelend.x + 2 * radius,
      crosschannelstart.y + channelWidth / 2
    );
    transposer_control.addChild(new paper.Path.Rectangle(topLeft, bottomRight));

    topLeft = new paper.Point(
      crosschannelend.x + 2 * radius - channelWidth / 2,
      crosschannelstart.y - channelWidth / 2
    );
    bottomRight = new paper.Point(
      crosschannelend.x + 2 * radius + channelWidth / 2,
    ...
    center = new paper.Point(
      px + 0.5 * channelWidth + valvespacing,
      py + 1.5 * channelWidth + 3 * valvespacing + 2 * radius + radius
    );
    circle = new paper.Path.Circle(center, radius);
    transposer_control.addChild(circle);
    ...
    // Bottom Row valve
    center = new paper.Point(
      px + channelWidth + 2 * valvespacing + radius,
      py + 4 * valvespacing + 4 * radius + 2 * channelWidth
    );
    circle = new paper.Path.Circle(center, radius);
    transposer_control.addChild(circle);

    topLeft = new paper.Point(
      px - 2 * radius - channelWidth / 2,
      py - 2 * radius - channelWidth / 2
    );
    bottomRight = new paper.Point(
      px - 2 * radius + channelWidth / 2,
      py + 4 * valvespacing + 6 * radius + 2 * channelWidth
    );
    transposer_control.addChild(new paper.Path.Rectangle(topLeft, bottomRight));

    topLeft = new paper.Point(
      px - 2 * radius - channelWidth / 2,
      py + 4 * valvespacing + 6 * radius + 2 * channelWidth - channelWidth / 2
    );
    bottomRight = new paper.Point(
      px + channelWidth + 2 * valvespacing + radius,
      py + 4 * valvespacing + 6 * radius + 2 * channelWidth + channelWidth / 2
    ...
    topLeft = new paper.Point(
      px + channelWidth + 2 * valvespacing + radius - channelWidth / 2,
      py + 4 * valvespacing + 4 * radius + 2 * channelWidth
    );
    bottomRight = new paper.Point(
      px + channelWidth + 2 * valvespacing + radius + channelWidth / 2,
    ...
    const topleftpoint = new paper.Point(
      px + valvespacing,
      py + channelWidth / 2 + 2 * radius + 2 * valvespacing
    );
    const bottomleftpoint = new paper.Point(
      topleftpoint.x + +4 * valvespacing + 3 * channelWidth + 4 * radius,
      topleftpoint.y + channelWidth
    );
    const rectangle = new paper.Path.Rectangle(topleftpoint, bottomleftpoint);
    transposer_control.addChild(rectangle);

    transposer_control.rotate(
      rotation,
      new paper.Point(
        px + 3 * valvespacing + 1.5 * channelWidth + 2 * radius,
        py + channelWidth + 2 * valvespacing + 2 * radius
