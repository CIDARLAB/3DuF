...
export default class CapacitanceSensor extends Template {
  ...
  getPorts(params: { [k: string]: any }) {
    ...
    ports.push(
      new ComponentPort(-inletLength / 2, 0, "1", LogicalLayerType.FLOW)
    );

    ports.push(
      new ComponentPort(inletLength / 2, 0, "2", LogicalLayerType.FLOW)
    );
  ...
  __renderIntegrate(params: { [k: string]: any }) {
    ...
    let topLeft = new paper.Point(
      x - electrodeWidth / 2,
      y - inletWidth / 2 - electrodeDistance - sensorWidth / 2 - electrodeLength
    );
    let bottomRight = new paper.Point(
      x + electrodeWidth / 2,
      y - inletWidth / 2 - electrodeDistance - sensorWidth / 2
    );
    ...
    let elli = new paper.Path.Ellipse({
      point: [
        x - sensorLength / 2,
        y - inletWidth / 2 - electrodeDistance - sensorWidth,
      ],
  ...
  render2D(params: { [k: string]: any }, key = "FLOW") {
    ...
    throw new Error(
      "Unknown render key found in DROPLET CAPACITANCE SENSOR: " + key
    );
  }
