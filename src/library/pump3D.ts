...
export default class Pump3D extends Template {
  ...
  getPorts(params: { [k: string]: any }) {
    ...
    ports.push(
      new ComponentPort(0, -spacing - radius, "1", LogicalLayerType.FLOW)
    );
    ports.push(
      new ComponentPort(0, spacing + radius, "2", LogicalLayerType.FLOW)
    );
  ...
  __drawFlow(params: { [k: string]: any }) {
    ...
    const bottomchannel = new paper.Path.Rectangle({
      from: new paper.Point(
        bottomcenter.x - channelwidth / 2,
        bottomcenter.y - gap / 2
      ),
