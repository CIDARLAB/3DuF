...
export default class DropletGenerator extends Template {
  ...
  render2D(params: { [k: string]: any }, key = "FLOW") {
    ...
    const p3 = new paper.Point(
      p2.x,
      p2.y + (waterInputWidth / 2 - orificeSize / 2)
    );

    const p4 = new paper.Point(p3.x + orificeLength, p3.y);

    const p5 = new paper.Point(
      p4.x,
      p4.y - (outputWidth / 2 - orificeSize / 2)
    );
    ...
    const p9 = new paper.Point(
      p8.x,
      p8.y - (outputWidth / 2 - orificeSize / 2)
    );

    const p10 = new paper.Point(p9.x - orificeLength, p9.y);

    const p11 = new paper.Point(
      p10.x,
      p10.y + (waterInputWidth / 2 - orificeSize / 2)
    );
