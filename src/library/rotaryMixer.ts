...
export default class RotaryMixer extends Template {
  ...
  getPorts(params: { [k: string]: any }) {
    ...
    const channellength =
      radius + valvelength + 2 * valvespacing + flowChannelWidth; // This needs to be a real expression
    ...
    ports.push(
      new ComponentPort(
        radius + flowChannelWidth + valvespacing + valvelength / 2,
        -radius - flowChannelWidth / 2 - valvewidth,
        "3",
    ...
    ports.push(
      new ComponentPort(
        flowChannelWidth / 2 + radius + valvewidth,
        0,
        "5",
        LogicalLayerType.CONTROL
      )
    ...
    ports.push(
      new ComponentPort(
        -radius - valvespacing - valvelength - flowChannelWidth / 2,
        radius + flowChannelWidth / 2 + valvewidth,
        "7",
        LogicalLayerType.CONTROL
      )
  ...
  __renderFlow(params: { [k: string]: any }) {
    ...
    let topleft = new paper.Point(
      px - valvelength / 2,
      py - radius - flowChannelWidth / 2 - valvewidth / 2
    );
    const topmiddlerectangle = new paper.Path.Rectangle(
      topleft,
      new paper.Size(valvelength, valvewidth)
    );
    rotary = rotary.subtract(topmiddlerectangle);

    topleft = new paper.Point(
      px + radius + flowChannelWidth / 2 - valvewidth / 2,
      py - valvelength / 2
    );
    const middlerightrectangle = new paper.Path.Rectangle(
      topleft,
      new paper.Size(valvewidth, valvelength)
    );
    ...
    const bottommiddlerectangle = new paper.Path.Rectangle(
      topleft,
      new paper.Size(valvelength, valvewidth)
    );
    ...
    let rectangle: paper.Path.Rectangle | paper.PathItem =
      new paper.Path.Rectangle(point1, point2);

    topleft = new paper.Point(
      px + radius + flowChannelWidth + valvespacing,
      py - radius - flowChannelWidth / 2 - valvewidth / 2
    );
    const topleftrectangle = new paper.Path.Rectangle(
      topleft,
      new paper.Size(valvelength, valvewidth)
    );
    ...
    let rectangle2: paper.Path.Rectangle | paper.PathItem =
      new paper.Path.Rectangle(point3, point4);

    topleft = new paper.Point(
      px - radius - valvespacing - valvelength - flowChannelWidth,
      py + radius + flowChannelWidth / 2 - valvewidth / 2
    );
    const bottomleftrectangle = new paper.Path.Rectangle(
      topleft,
      new paper.Size(valvelength, valvewidth)
    );
  ...
  __renderControl(params: { [k: string]: any }) {
    ...
    let topLeft = new paper.Point(
      ...
      py - radius - flowChannelWidth / 2 - valvewidth
    );
    ...
    topLeft = new paper.Point(
      px + flowChannelWidth / 2 + radius,
      py - controlChannelWidth / 2
    );
    bottomRight = new paper.Point(
      px + flowChannelWidth / 2 + radius + valvewidth,
      py + controlChannelWidth / 2
    );
    rotarymixer.addChild(new paper.Path.Rectangle(topLeft, bottomRight));
