...
export default class GradientGenerator extends Template {
  ...
  getPorts(params: { [k: string]: any }) {
    ...
    const stagelength =
      channelWidth * (2 * numBends + 1) +
      (2 * numBends + 2) * bendSpacing +
      channelWidth;

    const segBend = bendSpacing + 2 * channelWidth;

    const fullLength =
      (outvalue - 1) * (4 * segBend - 3.5 * channelWidth) + stagelength;
    ...
    for (let i = 0; i < invalue; i++) {
      ...
      ports.push(
        new ComponentPort(x, 0, (i + 1).toString(), LogicalLayerType.FLOW)
      );
    }
  ...
  render2D(params: { [k: string]: any }, key: string) {
    ...
    for (let stagevalue = invalue + 1; stagevalue <= outvalue; stagevalue++) {
      ...
      for (let i = 0; i < stagevalue; i++) {
        ...
        // insert the mixer
        this.__insertMixer(
          gradientgenerator as paper.Path,
          bendSpacing,
          numBends,
          channelWidth,
          bendLength,
