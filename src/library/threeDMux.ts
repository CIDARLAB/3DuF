...
export default class ThreeDMux extends Template {
  ...
  getPorts(params: { [k: string]: any }) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      ports.push(
        new ComponentPort(xpos, 0, (i + 1).toString(), LogicalLayerType.FLOW)
      );
    }
    ...
    for (let i = 0; i < 2 * valvenum; i++) {
      ...
      if (i % 2 === 0) {
        ...
        ports.push(
          new ComponentPort(
            leftInput,
            vertholder + (i * vertlinelength) / (2 * valvenum + 2),
            count.toString(),
  ...
  __drawFlow(params: { [k: string]: any }) {
    ...
    const bottomlineleft = new paper.Point(
      px,
      py - channelWidth / 2 + vertlinelength
    );
    const bottomlineright = new paper.Point(
      px + bottomlinelength,
      py + channelWidth / 2 + vertlinelength
    );
    ...
    for (let i = 0; i < N; i++) {
      ...
      const vertlinebottom = new paper.Point(
        px + xposbranch - channelWidth / 2,
        py + vertlinelength
      );
      const vertlinetop = new paper.Point(
        px + xposbranch + channelWidth / 2,
        py
      );
    ...
    // create output port
    const portCon = new paper.Point(
      px + bottomlinelength / 2 - channelWidth / 2,
      py + vertlinelength
    );
    const portOut = new paper.Point(
      px + bottomlinelength / 2 + channelWidth / 2,
      py + vertlinelength + N * 1000
    );
    ...
    for (let j = 0; j < valvenum; j++) {
      ...
      while (count1 < N) {
        for (let w = 0; w < cur_N / 2; w++) {
          const current_xpos =
            xpos + ((count1 + w) * bottomlinelength) / (N - 1);

          const cutrec = new paper.Path.Rectangle({
            from: new paper.Point(
              current_xpos - channelWidth / 2,
              ypos - gap / 2
            ),
            to: new paper.Point(
              current_xpos + channelWidth / 2,
      ...
      while (count2 < N) {
        for (let w = 0; w < cur_N / 2; w++) {
          ...
          branchArray[N - 1 - w - count2] =
            branchArray[N - 1 - w - count2].subtract(cutrec);
          this.__createthreedmuxValve(
            threedmux_flow,
            current_xpos,
            ypos,
  ...
  __drawControl(params: { [k: string]: any }) {
    ...
    for (let i = 0; i < 2 * valvenum; i++) {
      ...
      if (i % 2 === 0) {
        indexN /= 2;
        const cur_ind = N - indexN - 1;
        const leftsideLeft = new paper.Point(
          leftInput,
          py +
            vertholder +
            (i * vertlinelength) / (2 * valvenum + 2) -
            channelWidth / 2
        );
        const leftsideRight = new paper.Point(
          px + cur_ind * (bottomlinelength / (N - 1)),
          py +
            vertholder +
            (i * vertlinelength) / (2 * valvenum + 2) +
            channelWidth / 2
        );
        const leftcontrol = new paper.Path.Rectangle(
          leftsideLeft,
          leftsideRight
        );
      ...
      // right side
        ...
        const rightsideLeft = new paper.Point(
          px + cur_ind * (bottomlinelength / (N - 1)),
          py +
            vertholder +
            (i * vertlinelength) / (2 * valvenum + 2) -
        ...
        const rightsideRight = new paper.Point(
          rightInput,
          py +
            vertholder +
            (i * vertlinelength) / (2 * valvenum + 2) +
        ...
        const rightcontrol = new paper.Path.Rectangle(
          rightsideLeft,
          rightsideRight
        );
