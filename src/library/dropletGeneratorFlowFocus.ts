...
export default class DropletGeneratorFlowFocus extends Template {
  ...
  render2D(params: { [k: string]: any }, key: string) {
    ...
    let circ = new paper.Path.Circle(
      new paper.Point(x - length / 3 - 2 * length, y),
      radius
    );
    ...
    const seamCover =
      (oilChannelWidth / 2) * Math.tan((angleS * Math.PI) / 180);

    topLeft = new paper.Point(
      x - length / 3 - length - oilChannelWidth / 2,
      y - disFromMid - (2 * length) / 3 + seamCover
    );
    bottomRight = new paper.Point(
      x - length / 3 - length + oilChannelWidth / 2,
      y - disFromMid + seamCover
    );
    ...
    bottomRight = new paper.Point(
      x - length / 3 - length - oilChannelWidth / 2,
      y - disFromMid - (2 * length) / 3 + oilChannelWidth + seamCover
    );
    ...
    circ = new paper.Path.Circle(
      new paper.Point(
        x - length / 3 - 2 * length,
        y - disFromMid - (2 * length) / 3 + seamCover + oilChannelWidth / 2
      ),
      radius
    );
