...
export default class CellTrapS extends Template {
  ...
  getPorts(params: { [k: string]: any }) {
    ...
    ports.push(
      new ComponentPort(-channelLength / 2, 0, "1", LogicalLayerType.FLOW)
    );

    ports.push(
      new ComponentPort(channelLength / 2, 0, "2", LogicalLayerType.FLOW)
    );

    ports.push(
      new ComponentPort(0, -channelLength / 2, "3", LogicalLayerType.FLOW)
    );

    ports.push(
      new ComponentPort(0, channelLength / 2, "4", LogicalLayerType.FLOW)
    );
