...
export default class Filter extends Template {
  ...
  render2D(params: { [k: string]: any }, key: string) {
    ...
    const bodyWidth =
      filterNumber * filterWidth + (filterNumber - 1) * barrierWidth;
    ...
    let topRight = new paper.Point(
      x + inletLength + 2.5 * pillarDiameter,
      y - bodyWidth / 2
    );
    ...
    let circ = new paper.Path.Circle(
      new paper.Point(x + inletLength + 1.5 * pillarDiameter, y),
      pillarDiameter / 2
    );
    ...
    for (let i = 0; i < levelNumber; i++) {
      ...
      bottomRight = new paper.Point(
        x + startBody + filterLength + extraSpace,
        y + bodyWidth / 2
      );

      let rec: paper.PathItem | paper.Rectangle = new paper.Path.Rectangle(
        topLeft,
        bottomRight
      );
      ...
      if (i % 2 === 0) {
        const division =
          (bodyWidth - (filterNumber - 1) * barrierWidth) / filterNumber;
        let heightAccum = division;

        for (let j = 0; j < filterNumber - 1; j++) {
          topLeft = new paper.Point(
            x + startBody + extraSpace / 2,
            y - bodyWidth / 2 + heightAccum
          );
          heightAccum += barrierWidth;
          bottomRight = new paper.Point(
            x + startBody + filterLength,
            y - bodyWidth / 2 + heightAccum
          );
        ...
        const division =
          (bodyWidth - (filterNumber - 2) * barrierWidth) / (filterNumber - 1);
        let heightAccum = division;
    ...
    topRight = new paper.Point(
      x + startBody + 2.5 * pillarDiameter,
      y - outletWidth / 2
    );
    bottomRight = new paper.Point(
      x + startBody + 2.5 * pillarDiameter,
    ...
    circ = new paper.Path.Circle(
      new paper.Point(x + startBody - 1.5 * pillarDiameter, y),
      pillarDiameter / 2
    );
