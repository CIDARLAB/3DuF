  ...
  render2D(params: { [k: string]: any }, key: string) {
    ...
    for (let i = 0; i < numBends; i++) {
      ...
      let leftCurve: paper.Path.Arc | paper.PathItem = new paper.Path.Arc({
        ...
        through: [
          x + channelWidth - (channelWidth + bendSpacing / 2),
          y + vRepeat * i + bendSpacing / 2 + channelWidth,
        ],
        to: [
          x + channelWidth,
          y + vRepeat * i + bendSpacing + 2 * channelWidth,
      ...
      let hseg = new paper.Path.Rectangle(
        new paper.Rectangle(
          x + channelWidth - 1,
          y + vOffset + vRepeat * i,
          bendLength + 2,
      ...
      let rightCurve: paper.Path.Arc | paper.PathItem = new paper.Path.Arc({
        ...
        through: [
          x + channelWidth + bendLength + (channelWidth + bendSpacing / 2),
          y + vOffset + vRepeat * i + bendSpacing / 2 + channelWidth,
        ],
        to: [
      ...
      const rightCurveSmall = new paper.Path.Arc({
        from: [
          x + channelWidth + bendLength,
          y + vOffset + vRepeat * i + bendSpacing + channelWidth,
        ],
        through: [
          x + channelWidth + bendLength + bendSpacing / 2,
          y + vOffset + vRepeat * i + bendSpacing / 2 + channelWidth,
        ],
        to: [
          x + channelWidth + bendLength,
          y + vOffset + vRepeat * i + channelWidth,
        ],
      });
      ...
      if (i === numBends - 1) {
        ...
        hseg = new paper.Path.Rectangle(
          new paper.Rectangle(
            x + channelWidth / 2 + bendLength / 2,
            y + vRepeat * (i + 1),
            (bendLength + channelWidth) / 2 + 1,
            channelWidth
          )
        );
        toprect = toprect.unite(hseg);
