...
export default class Mux extends Template {
  ...
  __drawFlow(params: { [k: string]: any }) {
    ...
    return treepath.rotate(
      rotation,
      new paper.Point(px, py)
  ...
  __drawmuxcontroltwig(
    ...
    let startPoint = new paper.Point(
      lcenterx - valvewidth / 2,
      lcentery - valvelength / 2
    );
    let endPoint = new paper.Point(
      lcenterx + valvewidth / 2,
      lcentery + valvewidth / 2
    );
    let rec = new paper.Path.Rectangle({
      from: startPoint,
      to: endPoint,
    ...
    const leftChannelStart = new paper.Point(
      startPoint.x,
      lcentery - ctlcw / 2
    );
    ...
    startPoint = new paper.Point(
      rcenterx - valvewidth / 2,
      rcentery - valvelength / 2
    );
    endPoint = new paper.Point(
      rcenterx + valvewidth / 2,
      rcentery + valvewidth / 2
    );
    rec = new paper.Path.Rectangle({
      from: startPoint,
      to: endPoint,
