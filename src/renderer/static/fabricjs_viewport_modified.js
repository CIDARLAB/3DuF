...
(function() {
  ...
  fabric.Viewport = (function() {
    ...
    Viewport.prototype.centerOnCanvasPoint = function(point)
    {

    }

    Viewport.prototype.centerOnViewPoint = function(point){
      console.log("Centering on: " + point.x + ", " + point.y);
      half_x = this.canvas.width/2;
      x_diff = point.x - half_x;
      half_y = this.canvas.height/2;
      y_diff = point.y - half_y;
     // canvas_point = 
      this.position = new fabric.Point(this.position.x - x_diff, this.position.y - y_diff);
    }

    Viewport.prototype.viewToCanvas = function(point){
      diff_zoomed = new fabric.Point(point.x / this.zoom, point.y / this.zoom);
      return new fabric.Point(diff_zoomed.x - this.position.x / this.zoom, diff_zoomed.y - this.position.y / this.zoom);
    }

    Viewport.prototype.canvasToView = function(point){
      diff_zoomed = new fabric.Point(point.x * this.zoom, point.y * this.zoom);
      return new fabric.Point(diff_zoomed.x + this.position.x, diff_zoomed.y + this.position.y);
    }

    Viewport.prototype.grabEnd = function(e) {
      var diff;
      console.log("This start position: " + this.position);
      ...
      this.centerOnViewPoint(this.canvas.getPointer(e));
      //this.position = this.position.add(diff);
      console.log("Center: "+ this.canvas.width/2  + ", " + this.canvas.height/2);
      console.log("This position: " + this.position);
      console.log("Start grab (canvas): "+ this.viewToCanvas(this.grabStartPointer).x + ", " + this.viewToCanvas(this.grabStartPointer).y);
      console.log("Start grab (view): " + this.grabStartPointer.x + ", " + this.grabStartPointer.y);
      console.log("Start grab (redux): " + this.canvasToView(this.viewToCanvas(this.grabStartPointer)).x + ", " + this.canvasToView(this.viewToCanvas(this.grabStartPointer)).y);
      //this.position = new fabric.Point(this.canvas.getPointer(e).x, this.canvas.getPointer(e).y);
      this.lastGrab = new fabric.Point(this.grabStartPointer.x, this.grabStartPointer.y);
