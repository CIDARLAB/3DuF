...
var mode_description = [
    "0-Priming MCs, MCMs, MFMs and ALL Channels of the Board using MIX",
...
function addlist() {
    for (var i = mode_a.length - mode_new.length; i < mode_a.length; i++) {
        ...
        new_tr.innerHTML =
            ...
            "'>" +
            mode_a[i] +
            "</h5> </td> <td> <h4 id='port4-" +
            i +
            "'> " +
            point_path +
            "</h4> </td> <td style='width: 270px;'> <button onclick='check_each_mode(this)' class='button' id='check2-" +
            i +
            "' style='width:90px;float: left; margin-right: 10px;'> Check !</button> <button class='mode' id='remove" +
            i +
            "' onclick='remove_mode_tr(this)'>Del</button> <button class='diy' id='diy" +
            i +
            "' onclick='draw_diy(this)'>DIY it!</button> </td>";
