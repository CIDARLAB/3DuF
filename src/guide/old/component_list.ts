...
function creat_component_list() {
    ...
    for (let value of component_exist_list) {
        ...
        new_tr.innerHTML =
            "<tr> <td> <button class='button' id='port2' onclick='color_points(" +
            i +
            ")'>" +
            component_exist_list[i] +
            "</button> </td> <td> <h4 id='num" +
            i +
            "'>" +
            component_exist_num[i] +
            "</h4> </td> </tr>";
    ...
    new_tr = document.createElement("tr");
    new_tr.innerHTML =
        "<tr> <td> <button class='button' id='port2' onclick='color_points(" + i + ")'>Inlet</button> </td> <td> <h4 id='num" + i + "'>" + inlet.length + "</h4> </td> </tr>";
    document.getElementById("tb2").appendChild(new_tr);
}
