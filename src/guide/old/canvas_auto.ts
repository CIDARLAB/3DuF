...
function searchPath(
    path_info: Array<Component>,
    channel_flow: Array<Component>,
    block_valve: Array<number>,
    via_valve: Array<number>,
    mid_valve: Array<number>,
    chamber_valve: Array<number>,
    pump_block: Array<number>,
    num_port: number,
    level: number,
    mode_num: number
) {
        ...
        for (var j = 0; j < channel_flow.length; j++) {
            ...
            // last path may be a channel
                ...
                if (channel_flow[j].mint == "RoundedChannel") {
                    if (channel_flow[j].direct == path_info[t].direct) {
                        ...
                        if (path_info[t].direct == 1) {
                            ...
                            if (
                                (path_info[t].start[1] <= channel_flow[j].start[1] &&
                                    channel_flow[j].start[1] <= path_info[t].end[1] &&
                                    path_info[t].end[1] <= channel_flow[j].end[1] &&
                                    path_info[t].start[0] == channel_flow[j].start[0]) ||
                                (channel_flow[j].start[1] <= path_info[t].start[1] &&
                                    path_info[t].start[1] <= channel_flow[j].end[1] &&
                                    channel_flow[j].end[1] <= path_info[t].end[1] &&
                                    path_info[t].start[0] == channel_flow[j].start[0])
                            ) {
                        ...
                        // two lines are both horizontal
                            ...
                            if (
                                (path_info[t].start[0] <= channel_flow[j].start[0] &&
                                    channel_flow[j].start[0] <= path_info[t].end[0] &&
                                    path_info[t].end[0] <= channel_flow[j].end[0] &&
                                    path_info[t].start[1] == channel_flow[j].start[1]) ||
                                (channel_flow[j].start[0] <= path_info[t].start[0] &&
                                    path_info[t].start[0] <= channel_flow[j].end[0] &&
                                    channel_flow[j].end[0] <= path_info[t].end[0] &&
                                    path_info[t].start[1] == channel_flow[j].start[1])
                            ) {
                                path_info[path_info.length] = channel_flow[j];
                                channel_flow[j] = [];
                ...
                if (channel_flow[j].type == "Valve3D" || channel_flow[j].type == "Valve3D_control" || channel_flow[j].type == "Port" || channel_flow[j].type == "Port_control") {
                    ...
                    if (
                        block_valve.indexOf(channel_flow[j].id) != -1 ||
                        chamber_valve.indexOf(channel_flow[j].id) != -1 ||
                        via_valve.indexOf(channel_flow[j].id) != -1 ||
                        mid_valve.indexOf(channel_flow[j].id) != -1
                    )
                        continue;
                    ...
                    if (path_info[t].direct == 1) {
                        ...
                        if (
                            ((path_info[t].start[1] <= start && start <= path_info[t].end[1] && path_info[t].end[1] <= end) ||
                        ...
                        if (
                            ((path_info[t].start[0] <= start && start <= path_info[t].end[0] && path_info[t].end[0] <= end) ||
                ...
                if (channel_flow[j].type == "Pump3D" || channel_flow[j].type == "Pump3D_control") {
                    ...
                    if (level == 1) {
                        ...
                        if (path_info[t].direct == 1 && channel_flow[j].direct == 1) {
                            ...
                            if (
                                (path_info[t].start[1] <= start &&
                                    start <= path_info[t].end[1] &&
                                    path_info[t].end[1] <= end &&
                                    path_info[t].start[0] == channel_flow[j].position[0]) ||
                                (start <= path_info[t].start[1] &&
                                    path_info[t].start[1] <= end &&
                                    end <= path_info[t].end[1] &&
                                    path_info[t].start[0] == channel_flow[j].position[0])
                            ) {
                                path_ctrl_info[path_ctrl_info.length] = channel_flow[j];
                                path_info[path_info.length] = channel_flow[j];
                            ...
                            if (
                                (path_info[t].start[0] <= start &&
                                    start <= path_info[t].end[0] &&
                                    path_info[t].end[0] <= end &&
                                    path_info[t].start[1] == channel_flow[j].position[1]) ||
                                (start <= path_info[t].start[0] &&
                                    path_info[t].start[0] <= end &&
                                    end <= path_info[t].end[0] &&
                                    path_info[t].start[1] == channel_flow[j].position[1])
                            ) {
                                path_ctrl_info[path_ctrl_info.length] = channel_flow[j];
                                path_info[path_info.length] = channel_flow[j];
                        ...
                        if (path_info[t].direct == 0 && channel_flow[j].direct == 1) {
                            ...
                            if (
                                path_info[t].start[0] <= channel_flow[j].position[0] &&
                                channel_flow[j].position[0] <= path_info[t].end[0] &&
                                start <= path_info[t].start[1] &&
                                end >= path_info[t].end[1]
                            ) {
                                path_ctrl_info[path_ctrl_info.length] = channel_flow[j];
                            ...
                            if (
                                path_info[t].start[1] <= channel_flow[j].position[1] &&
                                channel_flow[j].position[1] <= path_info[t].end[1] &&
                                start <= path_info[t].start[0] &&
                                end >= path_info[t].end[0]
                            ) {
                                path_ctrl_info[path_ctrl_info.length] = channel_flow[j];
                    ...
                    if (path_info[t].direct == 1) {
                        ...
                        if (
                            (path_info[t].start[1] <= start &&
                                ...
                                path_info[t].end[1] <= end &&
                                path_info[t].start[0] == channel_flow[j].position[0]) ||
                            (start <= path_info[t].start[1] && path_info[t].start[1] <= end && end <= path_info[t].end[1] && path_info[t].start[0] == channel_flow[j].position[0])
                        ) {
                            path_info[path_info.length] = channel_flow[j];
                            channel_flow[j] = [];
                        }
            ...
            // last path may be a valve
                ...
                if (channel_flow[j].type == "RoundedChannel") {
                    ...
                    if (channel_flow[j].direct == 1) {
                        ...
                        if (
                            ((channel_flow[j].start[1] <= start && start <= channel_flow[j].end[1] && channel_flow[j].end[1] <= end) ||
                        ...
                        if (
                            ((channel_flow[j].start[0] <= start && start <= channel_flow[j].end[0] && channel_flow[j].end[0] <= end) ||
                ...
                if (channel_flow[j].type == "RoundedChannel") {
                    if (level == 1) {
                        ...
                        if (channel_flow[j].direct == 1 && path_info[t].direct == 1) {
                            ...
                            if (
                                (channel_flow[j].start[1] <= start &&
                                    start <= channel_flow[j].end[1] &&
                                    channel_flow[j].end[1] <= end &&
                                    channel_flow[j].start[0] == path_info[t].position[0]) ||
                                (start <= channel_flow[j].start[1] &&
                                    channel_flow[j].start[1] <= end &&
                                    end <= channel_flow[j].end[1] &&
                                    channel_flow[j].start[0] == path_info[t].position[0])
                            ) {
                                path_info[path_info.length] = channel_flow[j];
                                channel_flow[j] = [];
                            ...
                            if (
                                (channel_flow[j].start[0] <= start &&
                                    start <= channel_flow[j].end[0] &&
                                    channel_flow[j].end[0] <= end &&
                                    channel_flow[j].start[1] == path_info[t].position[1]) ||
                                (start <= channel_flow[j].start[0] &&
                                    channel_flow[j].start[0] <= end &&
                                    end <= channel_flow[j].end[0] &&
                                    channel_flow[j].start[1] == path_info[t].position[1])
                            ) {
                                path_info[path_info.length] = channel_flow[j];
                                channel_flow[j] = [];
